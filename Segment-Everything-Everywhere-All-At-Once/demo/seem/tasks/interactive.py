...
def interactive_infer_image(model, audio_model, image, tasks, refimg=None, reftxt=None, audio_pth=None, video_pth=None):
    ...
    if 'Panoptic' in tasks:
        ...
        tag_number = [] #category_idを格納
        for i in range(len(pano_seg_info)):
            pano_seg_number = pano_seg_info[i]['category_id']
            tag_number.append(pano_seg_number)

        tag_list = [] #タグ情報を格納
        for i in tag_number:
            tag_list.append(COCO_PANOPTIC_CLASSES[i])
        
        # tag_listをファイルに書き込む
        filename_without_extension = os.path.splitext(os.path.basename(image['image'].filename))[0]
        with open(output_file, "a") as file:
            file.write(f"{filename_without_extension}: {', '.join(tag_list)}\n")
    ...
    if 'Stroke' in tasks:
        ...
        print("pred_logits for Stroke:", pred_logits)  # ここに追加
        ...
        print("pred_logits for Example:", pred_logits)  # ここに追加
